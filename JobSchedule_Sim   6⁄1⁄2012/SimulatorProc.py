...
class SimulatorProc:
	...
	def beginSimulation(self):
		while (self.jobDoneNum_s < self.jobNum_s):
			...
			print str(self.waitNum_s)+"  " + str(self.runNum_s)+"  [Next Job: " + str(self.nextJob_s) +"  " + str(self.idleNode_s) +"]"
	...
	def jobStart(self,jobIndex_t2,waitIndex_t,startTime_t):				
		if (self.jobStartNum_s<self.jobNum_s):
			if (int(self.idleNode_s)>=int(self.jobList_s[jobIndex_t2]['reqNode'])):		
				...
				self.happyAnalysis(jobIndex_t2,startTime_t)
				print "  [idle: " +"  " + str(self.idleNode_s) +"]"
	...
	def collectSysUti(self):
		...
		if self.nextEventTime_s>0 or self.jobDoneNum_s == self.jobNum_s:
			...
			if (self.waitNum_s>0):
				tempDelta=1
			self.systemUti.append({'time':self.currentTime_s-self.startTime_s, 'inter':self.nextEventTime_s, 'uti':self.nodeNum_s-self.idleNode_s, 'delta':tempDelta})
			self.pre_uti=self.nodeNum_s-self.idleNode_s
			
	# Result Output Method
	def exportResult(self,outputName):
		outputName += ".rst"
