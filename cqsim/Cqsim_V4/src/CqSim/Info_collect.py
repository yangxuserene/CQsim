...
class Info_collect:
    ...
    def info_collect(self, time, event, uti, waitNum = -1, waitSize = -1, inter = -1.0, extend = None):
        ...
        temp_info = {'date': event_date, 'time': time, 'event': event, 'uti': uti, 'waitNum': waitNum, \
                     'waitSize': waitSize, 'inter': inter, 'extend': extend, 'ave_uti':[]}
                    
        self.sys_info.append(temp_info)
        if (len(self.sys_info)<=1):
            self.total_uti = 0
        else:
            self.total_uti += (event_date-self.sys_info[len(self.sys_info)-2]['time']) * self.sys_info[len(self.sys_info)-2]['uti']
            
        self.calculate_ave_uti()
        self.debug.debug("   "+"<"+str(self.total_uti)+">"+str(temp_info),2) 
    ...
    def reset_ave_uti_interval (self):
        ...
        if self.ave_uti_in == None:
            ...
            self.ave_uti_interval = self.ave_uti_in[:]
