...
class Cqsim_sim:
    ...
    def start(self, job_index):
        ...
        temp_node_req = {'proc':self.module['job'].job_info(job_index)['reqProc']}
        ...
        self.sys_collect({"type":1, "time":self.currentTime, "prio":0, "para":[3]})
        return
    ...
    def sys_collect(self,sys_info_list):
        ...
        if (self.event_pointer+1<len(self.event_seq)):
            temp_inter = sys_info_list['time'] - self.currentTime
        temp_size = 0
        ...
        self.module['info'].info_collect(time=self.currentTime, event=event_code,\
         ...
         waitNum=len(self.module['job'].wait_list()), waitSize=self.module['job'].wait_size(), inter=temp_inter, current_para = temp_para)
